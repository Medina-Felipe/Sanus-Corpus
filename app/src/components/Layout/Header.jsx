  ...
  return (
    <>
      <header className="bg-gradient-to-r from-[#E8F7FF] to-[#D1F0FF] w-full p-4 flex items-center justify-between shadow-sm">
        ...
        {/* Barra de Búsqueda*/}
        <form onSubmit={handleSearch} className="flex-1 mx-6 relative max-w-2xl">
          <input
            type="text"
          ...
          />
          <button type="submit" className="absolute left-4 top-1/2 transform -translate-y-1/2 text-[#0D99FF]">
        ...
        <div className="flex items-center space-x-3">
          {/* Botón Menú */}
          <button className="p-2 rounded-lg hover:bg-[#0D99FF]/10 transition-all duration-300" onClick={() => setIsMenuOpen(true)}>
            <img src={icon3} alt="Menú" className="h-8 w-8" />
          </button>
          {/* Botón Perfil */}
          <button className="p-2 rounded-full hover:bg-[#0D99FF]/10 transition-all duration-300" onClick={() => setIsMenuOpen(true)}>
          ...
          <button 
            className="p-2 rounded-lg hover:bg-[#0D99FF]/10 transition-all duration-300 relative"
